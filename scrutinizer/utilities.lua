...
function scrutinizer:IsPartyMemberByName(name,isnpc)
	if UnitClass("player") == TEXT("SYS_CLASSNAME_GM") and isnpc == 0 then
		return true
	elseif (name == UnitName(scrutinizer.playerID)) or InRaidByName(name) or InPartyByName(name) or (name == UnitName(scrutinizer.playerPetID)) then
		return true
	end
	return false
end
