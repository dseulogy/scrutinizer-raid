...
function scrutinizer:ResetBoss()

	for name, v in pairs(scrutinizer.combatants) do
		if scrutinizer.combatants[name][2] then
			scrutinizer.combatants[name][2] = nil
		end
...
function scrutinizer:PrevCat()
	if scrutinizer.currentcat > 1 then
		scrutinizer.currentcat = scrutinizer.currentcat - 1
	else
		scrutinizer.currentcat = #scrutinizer.cats
	end
	scrutinizer:SetCat()
end
...
function scrutinizer:OnEvent(this, event, ...)
	...
	elseif event == "COMBATMETER_DAMAGE" then
		if scrutinizer:IsPartyMemberByName(_source,_issourcenpc) and (_source ~= _target) then
			scrutinizer:AddCombatData(GetTime(), 1, _source, _damage, _target, _skill, _type)
			scrutinizer:AddCombatData(GetTime(), 2, _source, _damage, _target, _skill, _type)
		end
		if scrutinizer:IsPartyMemberByName(_target,_istargetnpc) then
			scrutinizer:AddCombatData(GetTime(), 3, _target, _damage, _source, _skill, _type)
		end
		scrutinizer.updateneeded = true
	elseif event == "COMBATMETER_HEAL" then
		if scrutinizer:IsPartyMemberByName(_source,_issourcenpc) then
			scrutinizer:AddCombatData(GetTime(), 4, _source, _heal, _target, _skill, _type)
		end
		if scrutinizer:IsPartyMemberByName(_target,_istargetnpc) then
			scrutinizer:AddCombatData(GetTime(), 5, _target, _heal, _source, _skill, _type)
		end
		scrutinizer.updateneeded = true
	else
...
function scrutinizer:rgbToHex(rgb)
	local hexadecimal = ''
...
function scrutinizer:UpdateAnchors()
	...
	if scrutinizer_MainFrame_Group:IsVisible() then
		scrutinizer:UpdateBarAnchor("scrutinizer_MainFrame_Group", "TOPLEFT", "BOTTOMLEFT", scrutinizer_MainFrame_Bar12)
		...
		if scrutinizer_MainFrame_Pet:IsVisible() then
			scrutinizer:UpdateBarAnchor("scrutinizer_MainFrame_Pet", "TOPLEFT", "BOTTOMLEFT", scrutinizer_MainFrame_Bar12)
		end
	end
...
function scrutinizer:UpdateMainFrame()
	...
	for name, v in pairs(scrutinizer.combatants) do
		if v[scrutinizer.currentcat] then
			if scrutinizer:IsPartyMemberByName(name,0) and not scrutinizer:IsPlayerPet(name) then
				table_insert(sorted, {
					name = name,
					...
					color = scrutinizer:rgbToHex( { v.r * 255, v.g * 255, v.b * 255 } ),
				})
